...
Recommended runtime: Python 3.12 or 3.13. Avoid Python alpha/beta builds for this skill because `requests`
loads `charset-normalizer`, and some pre-built `charset-normalizer` speedup wheels can crash the interpreter on
pre-release Python builds before Python can raise an exception.

If you must run on a Python pre-release, install the pure Python `charset-normalizer` wheel:

```bash
pip install requests beautifulsoup4 --no-binary charset-normalizer
```
...
If a script exits with `Segmentation fault (core dumped)` / exit code 139 before printing a Python traceback, check the
Python and `charset-normalizer` versions first:

```bash
python3 --version
python3 -m pip show requests charset-normalizer
```

This is usually a native extension load failure from `charset-normalizer`, not an application-level exception in the
search scripts. Fix it by using Python 3.12/3.13 or reinstalling dependencies with
`pip install requests beautifulsoup4 --no-binary charset-normalizer`.
